...
function sanitizeAvatar(raw) {
	...
	return {
		...
		spider_leg_count: Math.round(clamp(Number(raw.spider_leg_count) || 6, 4, 12)),
		eye_count: Math.round(clamp(Number(raw.eye_count) || 6, 2, 12)),
		eye_size: clamp(Number(raw.eye_size) || 1, 0.4, 3),
		eye_spread: clamp(Number(raw.eye_spread) || 1, 0.4, 2),
		eye_stalk_length: clamp(Number(raw.eye_stalk_length) || 0.35, 0, 1.5),
		mandible_length: clamp(Number(raw.mandible_length) || 1, 0, 2),
		fang_length: clamp(Number(raw.fang_length) || 0.8, 0, 2.5),
		claw_size: clamp(Number(raw.claw_size) || 0.6, 0, 2),
		abdomen_segments: clamp(Number(raw.abdomen_segments) || 0.5, 0, 1),
		crest_size: clamp(Number(raw.crest_size) || 0.25, 0, 1),
