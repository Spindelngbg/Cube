class_name CriminalBossSpawner
extends RefCounted
...
static func populate(
	...
	var root := Node3D.new()
	root.name = "CriminalBosses"
	parent.add_child(root)

	var plan := CriminalBossCatalogScript.get_npc_spawn_plan(spawn_id)
	if plan.is_empty():
		return root
	...
	for entry in plan:
		...
		npc.name = "Criminal_%s" % str(entry.get("id", spawn_index))
		...
		root.add_child(npc)
		npc.setup(entry, world_pos, hash("%s_criminal_%d" % [spawn_id, spawn_index]))
		if bool(entry.get("criminal_boss", false)):
			npc.add_to_group("criminal_boss_npc")
		elif bool(entry.get("criminal_henchman", false)):
			npc.add_to_group("criminal_henchman_npc")
