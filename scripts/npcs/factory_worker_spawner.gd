class_name FactoryWorkerSpawner
extends RefCounted
...
static func populate(
	...
	var root := Node3D.new()
	root.name = "FactoryWorkers"
	parent.add_child(root)

	var plan := FactoryWorkerCatalogScript.get_spawn_entries(spawn_id)
	for i in range(plan.size()):
		...
		root.add_child(npc)
		npc.setup(entry, world_pos, hash("%s_factory_worker_%d" % [spawn_id, i]))
		npc.add_to_group("factory_worker_npc")
